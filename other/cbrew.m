...
switch lower(ccmap_name)
    ...
    case lower('nph_CyclicRainbow')
        ccmap = [...
            0.72156862745098        0.0392156862745098         0.270588235294118
         0.890196078431372         0.145098039215686         0.152941176470588
         0.952941176470588         0.572549019607843         0.145098039215686
         0.905882352941176         0.737254901960784         0.164705882352941
         0.584313725490196         0.780392156862745         0.282352941176471
          0.12156862745098          0.72156862745098         0.650980392156863
         0.345098039215686         0.596078431372549         0.705882352941177
                       0.2         0.345098039215686         0.490196078431373
         0.431372549019608         0.207843137254902         0.486274509803922];
...
if len == 0 || size(ccmap,1) == len
    % leave it as its default length
    ccmapi = ccmap;
else
    % do something different for cyclic colormaps:
    if any(strcmpi(ccmap_name,{...
            'nph_CyclicIceFire',...
            'nph_CyclicEdge',...
            'nph_CyclicMRYBM',...
            'nph_CyclicMRGBM',...
            ...
            'nph_OSDiv1',...
            'nph_OSDiv2'}))
        % replicate ends and interpolate to make sure it loops nicely
        ccmapi = interp1(1:(3*size(ccmap,1)),repmat(ccmap,3,1),linspace(1,3*size(ccmap,1),3*len));
        ccmapi = ccmapi(len+(1:len),:);
    else
        % or just linearly interpolate it between its min/max as normal
