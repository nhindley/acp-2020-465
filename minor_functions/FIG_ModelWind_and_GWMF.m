...
load('/Users/neil/data/ModelWindvAlt.mat')
load('/Users/neil/data/upwp_vpwp_tp_average.mat')

% do stuff for wind plotting

new_zrange = 0:1:70;

[X,Y] = meshgrid(1:length(Wind.Time),Wind.Alt);
[XI,YI] = meshgrid(1:length(Wind.Time),new_zrange);

% Now interpolate to something useful:
Ui = interp2(X,Y,Wind.u,XI,YI);
Vi = interp2(X,Y,Wind.v,XI,YI);

% Split into years:
dv = datevec(Wind.Time);
...
ytix = {-100:20:100,-100:20:100,...
    -100:10:100,-100:10:100,...
    -100:10:100,-100:10:100,...
    };
yminortix = {-100:10:100,-100:10:100,...
    -100:5:100,-100:5:100,...
    -100:5:100,-100:5:100,...
    };
ylabs = {{'GWMF (mPa)','mPa'},...
...
for ax = 1:2
    ...
    % LINESPEC
    linespec = {'linewi',2.5,'color',rgbtrip(.2)};
    ...
    switch ax
        case {1,3,5}
            yl = ylabel({ylabs{ceil(ax/2)}{1},' '},'fontweight','bold');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%% SEPARATE RH AXES FOR FRACTION
    axx2 = axes('position',axx.Position);
    axx2.YAxisLocation = 'right';
    axx2.Color = 'none'; axx2.XColor = 'none'; axx2.XLim = axx.XLim;
    axx2.YLim = [-250 125];
    axx2.YTick = 0:25:100;
    axx2.YMinorTick = 'on';
    axx2.YAxis.MinorTickValues = 0:25:100;
    axx2.TickDir = 'out';
%     axx2.YMinorGrid = 'on';
    axx2.LineWidth = 1.5;
    axx2.GridLineStyle = '--';
...
%     if iseven(ax)
    axx2.YTickLabel = strtrim(cellstr([num2str(axx2.YTick') repmat('%',length(axx2.YTick),1)]));
    axx2.YTickLabel{2} = ' '; axx2.YTickLabel{4} = ' ';
    %     end
    
    frac = 100 .* (nansum(Q.B.totmf(inrange(M.z,zlims),:)) ./ nansum(Q.C.totmf(inrange(M.z,zlims),:)));
    frac = movmean(frac,smoo,2,'omitnan');
    
    hold on; plot(tim,frac(ti),linespec{:},'color','w','linewi',3);
    hold on; plot(tim,frac(ti),linespec{:},'color',rgbtrip(.5),'linewi',2,'linest',':');
    ...
    Q.C.MF1 = 1000*Dens.*Q.C.vpwp;
    Q.C.MF2 = 1000*Dens.*Q.C.upwp;
    
    for q = [2 1]
        
        switch q
            case 1
                data = movmean(nanmean(Q.C.MF2(inrange(M.z,zlims),:)),smoo,2,'omitnan');
                hold on; plot(tim,data(ti),linespec{:},'color',mcolor(1));
            case 2
                data = movmean(nanmean(Q.C.MF1(inrange(M.z,zlims),:)),smoo,2,'omitnan');
                hold on; plot(tim,data(ti),linespec{:},'color',mcolor(2));
        end
    end
    
    %%%% LEGEND
%     L = legend({'$\rho\,u''w''$','$\rho\,v''w''$'});
    L = legend({'Zonal MF','Meridional MF'});
...
%     L.Interpreter = 'latex';
    ...
    drawnow;
    nph_text([-0.02 1.05],['(' alphabet(ax+4) ')'],'fontsize',1.75*fs,'HorizontalAlignment','center','textborder','w');
    drawnow;
...
savename = ['~/Desktop/Model_Timeseries_ZonalMeridMF'];
...
save([winddirec '/ModelWindvAlt.mat'],'Wind')
...
ytix = {0:25:100,0:25:100,...
    -100:10:100,-100:10:100,...
    -100:10:100,-100:10:100,...
    };
yminortix = {0:5:100,0:5:100,...
    -100:5:100,-100:5:100,...
    -100:5:100,-100:5:100,...
    };
...
modelasairsdirec    = '/Volumes/SDBlue/data/sgwex/3DST/Model_as_AIRS/AG/';
winddirec           = '/Volumes/SDBlue/data/sgwex/VertWindProfiles/';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% TIMESERIES OF WAVE AMPLITUDE, ZONAL GWMF, MERIDIONAL GWMF
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for ax = 1:6
    ...
    for q = [1 2]
        ...
        switch ax
            case {1,2}
                data = movmean(Q.(region).amedian(ti),smoo,2,'omitnan');
            case {3,4,5,6}
...
savename = ['~/Desktop/AirsModel_Timeseries_A_MFx_MFy'];
disp(['Exporting to ' savename '...'])
...
savename = ['~/Desktop/AirsModel_Timeseries_FracAB'];
disp(['Exporting to ' savename '...'])
